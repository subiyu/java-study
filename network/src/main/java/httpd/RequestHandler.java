...
public class RequestHandler extends Thread {
	...
	final String DOCUMENT_ROOT = "./webapp";
	...
	public void run() {
		try {
			...
			while(true) {				
				...
				// SimpleHttpServer는 HTTP header 만 처리 
				if("".equals(line)) {
					break;
				}
			...
			if("GET".equals(tokens[0])) {
				 ...
				 * response400BadReqeust(outputStream, tokens[2]);
				 */
				response400BadReqeust(outputStream, tokens[2]);
			}
		} catch( Exception ex ) {
	...
	private void response400BadReqeust(OutputStream outputStream, String protocol) throws UnsupportedEncodingException, IOException {
